			...
			if response["response_code"] == 200:
				try:
					...
					if len(response["results"]["detected_urls"]) == 0:
						...
						table += "  <tr>\n"
						#NOU CAMP (COLUMNA) a la FILA
						table += """<th scope="row">{0}</th>""".format("Malicious")
						table += """<td>{0}</td>\n""".format("False")
						#TANCO FILA
						table += "  </tr>\n"
						...
						table += "  <tr>\n"
						#NOU CAMP (COLUMNA) a la FILA
						table += """<th scope="row">{0}</th>""".format("Malicious URLs")
						table += "<td>"
					...
					table += "  <tr>\n"
					#NOU CAMP (COLUMNA) a la FILA
					table += """<th scope="row">{0}</th>""".format("Domain Resolutions")
					table += "<td>"
					for resolution in response["results"]["resolutions"]:
						table += """<p>{0} ({1})</p>""".format(resolution["hostname"], resolution["last_resolved"])
					table += "</td>"
...
def locateIPHTML(ip):
	...
	try:
		with geoip2.webservice.Client(534778, '2KcXWF0vdaOxe6dL') as client:
			...
			print("IP: " + str(ip) + " --> Latitude: " + str(response.location.latitude))
			print("IP: " + str(ip) + " --> Longitude: " + str(response.location.longitude))
