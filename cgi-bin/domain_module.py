...
def whois(domain):
    ...
    try:
        if json_whois['domain_registered'] != 'no':
            ...
            print(" -> Domain " + json_whois['domain_name'] + " was registered on " +
                  json_whois['create_date'] + " in " + json_whois['domain_registrar']['registrar_name'])
            print(" -> Name servers")
...
def whois_history(domain):
    ...
    print(" [*] Found " +
          str(json_whois_history['total_records_found']) + " result(s)")
        ...
        for c, i in enumerate(json_whois_history['whois_records']):
            try:
                ...
                print(" -> Domain " + domain + " was registered on " +
                      i['update_date'] + " in " + i['domain_registrar']['registrar_name'])
                    ...
                    if response["results"]["detected_downloaded_samples"]:
                        if (response["results"]["detected_downloaded_samples"] > 0):
                            for sample in response["results"]["detected_downloaded_samples"]:
                                print(
                                    " --> " + sample["positives"] + " antivirus detect it as malicious at " + sample["date"])
                        else:
                            #print("[+] Domain: " + domain)
                            print("  --> Legit domain!!!")
                except:
