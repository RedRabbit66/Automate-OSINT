...
def whois_html(domain):
	...
	<section class="ftco-section">
		<div class="container">
			...
			table += """<td>{0} in {1}</td>\n""".format(json_whois['create_date'], json_whois['domain_registrar']['registrar_name'])
			...
			for k in json_whois['registrant_contact']:
				table += """<p>{0}</p>""".format(json_whois['registrant_contact'][k])
			...
			<section class="ftco-section">
				<div class="container">
					...
					for k in i['registrant_contact']:
						table += """<p>{0}</p>""".format(i['registrant_contact'][k])
					...
					print("[*] Domain " + domain + " was registered on " + i['create_date'] + " in " + i['domain_registrar']['registrar_name'])
		...
		if response["response_code"]:
			...
			if response["response_code"] == 200:
				try:
					...
					#NOVA FILA
					table += "  <tr>\n"
					table += """<th scope="row">{0}</th>""".format(response["results"]["Webutation domain info"]["Verdict"])
					table += """<td>{0}</td>""".format(response["results"]["Webutation domain info"]["Adult content"])
					table += """<td>{0}</td>""".format(response["results"]["Webutation domain info"]["Safety score"])
	...
	if res.status_code == 200:
		...
		try:
			...
			for selector in json_response["selectors"]:
				#NOVA FILA
				table += "  <tr>\n"
				table += """<th scope="row">{0}</th>""".format(selector["selectorvalue"])
