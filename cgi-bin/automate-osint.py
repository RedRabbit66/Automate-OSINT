...
def parseArgs():
	parser = argparse.ArgumentParser(prog='automate-osint.py', usage='%(prog)s [options] path',description='Automated-OSINT is used to gather open source intelligence (OSINT) on diferent aspects.')
	parser.add_argument('-e', '--email', help='Email to search.', required=False, type=str, default="")
	parser.add_argument('-d', '--domain', help='Company name or domain to search.', required=False, type=str, default="")
	parser.add_argument('-u', '--username', help='Username to search.', required=False, type=str, default="")
	parser.add_argument('-i', '--ip', help='IP to search.', required=False, type=str, default="")
	parser.add_argument('-p', '--password', help='Password to search.', required=False, type=str, default="")
	parser.add_argument('-w', '--wallet', help='BTC wallet to search.', required=False, type=str, default="")
	parser.add_argument('-t', '--term', help='BTC wallet to search.', required=False, type=str, default="")
	parser.add_argument('-html', '--html', help='Enable HTML output, default False.', default=False, action='store_true')
	global arguments
...
def main():
	...
	if email:
		if(email and (re.match(email_pattern, email))):
			if not html_output:
				...
				contingut_HTML = contingut_HTML + email_html.get_gravatar_info_html(email)
				contingut_HTML = contingut_HTML + email_html.sitesUsedByTarget(email)
				contingut_HTML = contingut_HTML + email_html.get_breachdirectory_html(email)
				contingut_HTML = contingut_HTML + email_html.pastes_search_html(email)
				contingut_HTML = contingut_HTML + email_html.psbdmp_search_html(email)
				contingut_HTML = contingut_HTML + email_html.get_darknet_leak(email)
				contingut_HTML = contingut_HTML + email_html.leaksDBs(email)

		else:
			print("[-] This is not an email!")
			returnIndex("This is not an email! Redirecting you to the main page.")
	...
	if domain:
		if (domain and (re.match(domain_pattern, domain))):
			if not html_output:
				...
				contingut_HTML = contingut_HTML + domain_html.whois_html(domain)
				contingut_HTML = contingut_HTML + domain_html.whois_history_html(domain)
				contingut_HTML = contingut_HTML + domain_html.virustotal_domain_html(domain)
				contingut_HTML = contingut_HTML + domain_html.domainReputationHTML(domain)
				contingut_HTML = contingut_HTML + domain_html.get_darknet_leak(domain)
				contingut_HTML = contingut_HTML + domain_html.getEmails(domain)
				
		else:
			print("[-] This is not a domain!")
			returnIndex("This is not a domain! Redirecting you to the main page.")

	if username:
		if html_output:
			username_html.parseConfig()
			contingut_HTML = contingut_HTML + username_html.sherlock_finder(username)
			contingut_HTML = contingut_HTML + username_html.psbdmp_search_html(username)
			contingut_HTML = contingut_HTML + username_html.pastes_search_html(username)
			contingut_HTML = contingut_HTML + username_html.get_darknet_leak(username)
		else:
	...
	if password:
		if html_output:
			...
			contingut_HTML = contingut_HTML + password_html.commonPasswordChecker(password)
			contingut_HTML = contingut_HTML + password_html.pastes_search_html(password)
			contingut_HTML = contingut_HTML + password_html.psbdmp_search_html(password)
			contingut_HTML = contingut_HTML + password_html.leakedPasswordChecker(password)
			contingut_HTML = contingut_HTML + password_html.get_darknet_leak(password)
		else:
	...
	if wallet:
		if not html_output:
			...
			contingut_HTML = contingut_HTML + wallet_html.wallet_report_html(wallet)
			contingut_HTML = contingut_HTML + wallet_html.url_wallet_info_html(wallet)
