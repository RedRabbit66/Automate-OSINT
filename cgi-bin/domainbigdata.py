...
class DomainBigData:
        ...
        for site in related_sites:
            if idx == 0:
                ...
                associated_sites.append(
                    {'associated_site': associated_site, 'creation_date': creation_date, 'registrar': registrar})
                continue
    ...
    def collect_registrant_information(self, soup):
        ...
        organization_soup = soup.findAll(
            'tr', {'id': 'MainMaster_trRegistrantOrganization'})
        ...
        if len(organization_soup) == 1:
            registrant_organization = self.extract_information_from_registrant(
                organization_soup[0])
            orgdom = self.extract_information_from_dd(organization_soup[0])
            self.intelligence['Domain_For_Org'] = orgdom

        if len(email_soup) == 1:
            registrant_email = self.extract_information_from_registrant(
                email_soup[0])
            emaildom = self.extract_information_from_dd(email_soup[0])
            self.intelligence['Domain_For_Email'] = emaildom

        if len(name_soup) == 1:
            registrant_name = self.extract_information_from_registrant(
                name_soup[0])
            namedom = self.extract_information_from_dd(name_soup[0])
            self.intelligence['Domain_For_Name'] = namedom

        if len(city_soup) == 1:
            registrant_city = self.extract_information_from_registrant(
                city_soup[0])

        if len(country_soup) == 1:
            registrant_country = self.extract_information_from_registrant(
                country_soup[0])

        if len(phone_soup) == 1:
            registrant_phone = self.extract_information_from_registrant(
                phone_soup[0])
    ...
    def extract_information_from_dd(self, soup):
        ...
        # get dom
        if link:
            domains = self.name2dom_collect_information(
                "http://%s%s" % (self.host, link))
            return {name: domains}
